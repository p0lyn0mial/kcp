...
func NewMaximalPermissionPolicyAuthorizer(kubeInformers kcpkubernetesinformers.SharedInformerFactory, kcpInformers kcpinformers.SharedInformerFactory, delegate authorizer.Authorizer) (authorizer.Authorizer, error) {
	...
	return &MaximalPermissionPolicyAuthorizer{
		getAPIBindingReferenceForAttributes: func(attr authorizer.Attributes, clusterName logicalcluster.Path) (*apisv1alpha1.ExportReference, bool, error) {
		...
		newAuthorizer: func(clusterName logicalcluster.Path) authorizer.Authorizer {
			return rbac.New(
...
type MaximalPermissionPolicyAuthorizer struct {
	...
	getAPIBindingReferenceForAttributes func(attr authorizer.Attributes, clusterName logicalcluster.Path) (ref *apisv1alpha1.ExportReference, found bool, err error)
...
func (a *MaximalPermissionPolicyAuthorizer) Authorize(ctx context.Context, attr authorizer.Attributes) (authorizer.Decision, string, error) {
	...
	if bindingLogicalCluster.Cluster != nil {
		...
		cluster = string(bindingLogicalCluster.Cluster.Identifier)
	}
...
func getAPIBindingReferenceForAttributes(apiBindingClusterLister apisv1alpha1listers.APIBindingClusterLister, attr authorizer.Attributes, clusterName logicalcluster.Path) (*apisv1alpha1.ExportReference, bool, error) {
...
func getAPIExportByReference(apiExportClusterLister apisv1alpha1listers.APIExportClusterLister, exportRef *apisv1alpha1.ExportReference) (*apisv1alpha1.APIExport, bool, error) {
	objs, err := apiExportClusterLister.Cluster(exportRef.Cluster.Identifier.Path()).List(labels.Everything())
	if err != nil {
		return nil, false, err
	}
