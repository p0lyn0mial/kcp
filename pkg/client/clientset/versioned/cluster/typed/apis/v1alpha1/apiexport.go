...
func (c *aPIExportsClusterInterface) Cluster(path logicalcluster.Path) apisv1alpha1client.APIExportInterface {
	if path == logicalcluster.WildcardPath {
		panic("A specific cluster must be provided when scoping, not the wildcard.")
	}

	return c.clientCache.ClusterOrDie(path).APIExports()
}
...
func (c *aPIExportsClusterInterface) List(ctx context.Context, opts metav1.ListOptions) (*apisv1alpha1.APIExportList, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.WildcardPath).APIExports().List(ctx, opts)
}
...
func (c *aPIExportsClusterInterface) Watch(ctx context.Context, opts metav1.ListOptions) (watch.Interface, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.WildcardPath).APIExports().Watch(ctx, opts)
}
