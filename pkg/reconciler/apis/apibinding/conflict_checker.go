...
func (ncc *conflictChecker) getBoundCRDs(apiBindingToExclude *apisv1alpha1.APIBinding) error {
	...
	for _, apiBinding := range apiBindings {
		...
		apiExportClusterName := apiBinding.Spec.Reference.Cluster.Identifier

		apiExport, err := ncc.getAPIExport(apiExportClusterName, apiBinding.Spec.Reference.Cluster.ExportName)
		if err != nil {
			return err
		}
