...
func (c *cowboysClusterInterface) Cluster(path logicalcluster.Path) CowboysNamespacer {
	if path == logicalcluster.WildcardPath {
		panic("A specific cluster must be provided when scoping, not the wildcard.")
	}

	return &cowboysNamespacer{clientCache: c.clientCache, path: path}
}
...
func (c *cowboysClusterInterface) List(ctx context.Context, opts metav1.ListOptions) (*wildwestv1alpha1.CowboyList, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.WildcardPath).Cowboys(metav1.NamespaceAll).List(ctx, opts)
}
...
func (c *cowboysClusterInterface) Watch(ctx context.Context, opts metav1.ListOptions) (watch.Interface, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.WildcardPath).Cowboys(metav1.NamespaceAll).Watch(ctx, opts)
}
...
func (n *cowboysNamespacer) Namespace(namespace string) wildwestv1alpha1client.CowboyInterface {
	return n.clientCache.ClusterOrDie(n.path).Cowboys(namespace)
}
